...
app.get('/profile/interests', async(req, res) => {
  ...
  if (currentUser){
    ...
    res.send({skills: userSkills, skillsJson: skillsJson.skills, companies:userCompanies, companyJson: companyJson.companies, languages: userLanguages, message: "got user interests ", typeStatus:"success" })
  }
  else{
...
function getScore(skills, roles){
  const skillScore = calculateScore(skills, "category", "name", skillWeight, skillNameWeight);
...
async function getMatches(currentUser){
  ...
  entries.forEach(async entry => {
    ...
    const skillsInfo = {user_1: currentUserInfo.skills, user_2: matchInfo.skills};
    const rolesInfo = {user_1: currentUserInfo.roles, user_2: matchInfo.roles};
...
async function getMatchData(limit, offset, currentUser){
  ...
  for (let i = 0 ; i < results.length; i++){
    ...
    scoreInfo.push({ score: results[i].get('score'), liked: results[i].get('liked'), display_private: results[i].get('display_private')});
  ...
  return({ matchesInfo: matchesInfo, results: results, message: "matches found!", typeStatus: "success"})
}
